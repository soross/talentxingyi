...
public class NIOThread implements Runnable {
	 ...
	 *         如果注册失败
	 */
	public void register(IConnection iConnection) throws ClosedChannelException {
		SelectableChannel channel = iConnection.channel();
		if (channel instanceof SocketChannel) {
			channel.register(selector, SelectionKey.OP_CONNECT, iConnection.getNIOHandler());
		} else if (channel instanceof DatagramChannel) {
			channel.register(selector, SelectionKey.OP_READ, iConnection.getNIOHandler());
		}
		if (!ports.contains(iConnection))
			ports.add(iConnection);
	}
