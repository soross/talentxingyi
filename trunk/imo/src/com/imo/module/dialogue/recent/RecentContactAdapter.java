...
public class RecentContactAdapter extends BaseAdapter {
		...
		if (info != null) {
			if (isCategory) {
				...
				if (isEditState) {
					...
					// bind close event
					recentContactView.setCloseListene(new CloseListener(position, info.getCid(), info.getType(), info.getUid(), info.getTime().split(" ")[0]));
				} else {
	...
	class CloseListener implements View.OnClickListener {
		...
		private void doDelUserContactRecord(final int position, final int cid, final int type, final int uid) {
			...
			try {
				IMOApp.getApp();
				// /删除数据库中最近联系人的记录
				result = IMOApp.imoStorage.deleteRecentContact(type, uid);
			} catch (Exception e) {
	...
	private void setFace(RecentContactView recentContactView, RecentContactInfo info) {
		...
		boolean isBoy = ((RecentContactActivity) mContext).getNodeByUid(info.getUid()) != null ? ((RecentContactActivity) mContext).getNodeByUid(info.getUid()).getNodeData().isBoy : false;
