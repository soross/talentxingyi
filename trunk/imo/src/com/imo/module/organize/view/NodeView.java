...
public class NodeView extends RelativeLayout {
	...
	private void init(Context context, int type) {
		...
		// int size = (int)
		// (mContext.getResources().getDimension(R.dimen.dept_node_fontsize) *
		 ...
		 * (ViewGroup.LayoutParams)getLayoutParams(); // params.width = (int)
		 * (width * IMOApp.getApp().mScale); params.height = (int) (height *
		 * IMOApp.getApp().mScale); //在原有的高度 宽度放大倍数
