...
public class DialogFactory {
	 ...
	 * @return
	 */
	public static AlertDialog alertDialog(Context context, String title, String msg, String[] btnNames, DialogInterface.OnClickListener... listeners) {
	...
	public static AlertDialog promptExit(final Context context) {
		...
		builder.setPositiveButton("退出", new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface arg0, int arg1) {
				IMOApp.getApp().setAppExit(true);
				Functions.backToDesk(IMOApp.getApp().mLastActivity);
				if (EngineConst.isNetworkValid && AppService.getService().getTcpConnection().isConnected()) {
					((AbsBaseActivityNetListener) IMOApp.getApp().mLastActivity).requestLogOut(true);
				} else {
					IMOApp.getApp().exitApp();
				}
			}
	 ...
	 * 电话选择对话框
	 */
	public static AlertDialog selectMobile(final Context context, String mobile, String tel, DialogInterface.OnClickListener listener) {
		return new AlertDialog.Builder(context).setTitle("号码列表").setItems(new String[] {
				"手机：" + Functions.formatPhone(mobile), "电话：" + tel
	...
	public static AlertDialog doCall(final Context context, final String tel) {
		...
		builder.setPositiveButton("拨打", new DialogInterface.OnClickListener() {
			public void onClick(DialogInterface arg0, int arg1) {
				Intent myIntentDial = new Intent(Intent.ACTION_CALL, Uri.parse("tel:" + tel));
	 ...
	 * 复制对话框
	 */
	public static AlertDialog copyDialog(final Context context, DialogInterface.OnClickListener listener) {
		return new AlertDialog.Builder(context).setTitle("操作选项").setItems(new String[] {
			"复制"
		}, listener).show();
	}
}
