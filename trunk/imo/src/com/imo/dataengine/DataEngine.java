...
import com.imo.network.packages.BatchGetQGroupUserListMulretInPacket;
import com.imo.network.packages.BatchGetUserQGroupListMulretInPacket;
...
import com.imo.network.packages.ErrorPacket;
...
import com.imo.network.packages.GetOfflineMsgInPacket;
...
import com.imo.network.packages.inpak.GetClientLoginNgroupdInPacket;
...
import com.imo.network.packages.inpak.GetModifyNgroupNameNoticeInPacket;
...
import com.imo.network.packages.inpak.GetNgroupInfoInPacket;
...
import com.imo.network.packages.inpak.GetUserNgGroupListUcNewInPacket;
...
public class DataEngine {
	...
	public void removePacketFromTimeoutQueue(int aSeq) {
		LogFactory.e("removePacketFromTimeoutQueue",
				"removePacketFromTimeoutQueue seq = :" + aSeq);
	...
	public void reconnectServer() {
		if (current_logic_status == LOGICSTATUS.DISCONNECTED) {
			...
			AppService.getService().setTcpConnection(
					(TCPConnection) AppService
							.getService()
							.getNIOThreadInstance()
							.newTCPConnection(EngineConst.IMO_CONNECTION_ID,
									EngineConst.IMO_SERVER_ADDRESS, true));

			AppService.getService().getNIOThreadInstance()
					.release(oldConnectionID, false);

			ReloginOutPacket out = new ReloginOutPacket(
					ByteBuffer.wrap(EngineConst.sessionKey),
					IMOCommand.IMO_GET_RELOGIN, EngineConst.cId,
					EngineConst.uId);
			AppService.getService().getNIOThreadInstance()
					.send(EngineConst.IMO_CONNECTION_ID, out, false);
		}
	}

	private Object lock_observerNotifyPacketFailed = new Object();

	public void observerNotifyPacketFailed(String aConnectionId,
			short aErrorCode) {
	...
	public void observerNotifyPacketTimeOut(String aConnectionId,
			short aErrorCode) {
	...
	public void observerNotifyPacketArrived(String aConnectionId, short command) {
		synchronized (lock_observerNotifyPacketArrived) {
			...
			while (itrObserver.hasNext()) {
				...
				if (command == IMOCommand.IMO_FORCE_EXIT
						|| command == IMOCommand.IMO_HEART_BEAT
						|| command == IMOCommand.IMO_SEND_MESSAGE
						|| command == IMOCommand.IMO_MODIFY_QGROUP_NAME_NOTICE
						|| command == IMOCommand.IMO_GET_RELOGIN
						|| command == IMOCommand.IMO_LOGIN
						|| command == IMOCommand.IMO_EXIT_QGROUP_NOTICE_USERS
						|| command == IMOCommand.IMO_QGOURP_CHAT
						|| command == IMOCommand.IMO_UPDATE_VERSION
						|| command == IMOCommand.IMO_QGROUP_DESTROY_NOTICE_USER
						|| command == IMOCommand.IMO_NEW_MSG_NOTICE_QGROUP
						|| command == IMOCommand.IMO_QGROUP_USER_CHANGE_STATUS_NOTICE
						|| command == IMOCommand.IMO_QGROUP_NOTICE
						|| command == IMOCommand.IMO_QGROUP_KICK_USER_NOTICE
						|| command == IMOCommand.IMO_NOTICE_USER_JOIN_QGROUP
						|| command == IMOCommand.IMO_QGROUP_MODIFY_ANNOUNCEMENT_NOTICE
						|| command == IMOCommand.IMO_SERVER_PROMPT) {
					LogFactory.e("observerNotifyPacketArrived", "" + command);
					observer.NotifyPacketArrived(aConnectionId, command);
					break;
				} else {
					if (((AbsBaseActivityNetListener) (IMOApp.getApp().mLastActivity))
							.CanAcceptPacket(command)) {
						((AbsBaseActivityNetListener) (IMOApp.getApp().mLastActivity))
								.NotifyPacketArrived(aConnectionId, command);
						break;
					}
	...
	public void observerNotifyHttpPacketArrived(String aConnectionId,
			ByteBuffer buffer) {
	}

	private Object lock_observerNotifyPacketProgress = new Object();

	public void observerNotifyPacketProgress(String aConnectionId,
			short command, short aTotalLen, short aSendedLen) {
		synchronized (lock_observerNotifyPacketProgress) {
			Iterator<PacketsObserver> itrObserver = observerQueue.iterator();
			while (itrObserver.hasNext()) {
				PacketsObserver observer = itrObserver.next();
				if (observer.CanAcceptPacket(command)) {
					observer.NotifyPacketProgress(aConnectionId, command,
							aTotalLen, aSendedLen);
					break;
				}
			}
		}
	...
	public InPacket parseIn(ByteBuffer buf, int length)
	...
	public InPacket parseIncoming(ByteBuffer buf, int offset, int length)
			throws PacketParseException {
		short commandRet = getCommand(buf, offset);

		LogFactory.e("parseIncoming packet: ", "" + commandRet);
		ConnectionLog.MusicLogInstance().addLog(
				"parseIncoming packet: ," + commandRet);
		...
		case IMOCommand.IMO_GET_ALL_EMPLOYEE_UID:
			GetAllEmployeesUIDInPacket employeesuidInPac = new GetAllEmployeesUIDInPacket(
					buf, length);
		...
		case IMOCommand.IMO_GET_ALL_EMPLOYEE_BASIC_INFO:
			GetAllEmployeesInfoInPacket employeesbasicinfoInPac = new GetAllEmployeesInfoInPacket(
					buf, length);
			employeesbasicinfoInPac
					.setCommand(IMOCommand.IMO_GET_ALL_EMPLOYEE_BASIC_INFO);
			removePacketFromTimeoutQueue(employeesbasicinfoInPac
					.get_header_seq());
			return employeesbasicinfoInPac;
		case IMOCommand.IMO_GET_EMPLOYEE_STATUS:
			GetEmployeesStatusInPacket employeesStatusInPac = new GetEmployeesStatusInPacket(
					buf, length);
		...
		case IMOCommand.IMO_SEND_MESSAGE:
			...
			// LogFactory.e("MsgContent:", ""+sendmsgInPac.getMsg());
			// ConnectionLog.MusicLogInstance().addLog("MsgContent:"+sendmsgInPac.getMsg());
		...
		case IMOCommand.IMO_GET_OFFLINE_MSG:
			GetOfflineMsgInPacket offlinemsgInPac = new GetOfflineMsgInPacket(
					buf, length);
		...
		case IMOCommand.IMO_GET_EMPLOYEE_PROFILE:
			GetEmployeeProfileInPacket employeeprofInPac = new GetEmployeeProfileInPacket(
					buf, length);
		...
		case IMOCommand.IMO_UPDATE_USER_STATUS:
			UserStatusChangeInPacket statusChangeInPac = new UserStatusChangeInPacket(
					buf, length);
			statusChangeInPac.setCommand(IMOCommand.IMO_UPDATE_USER_STATUS);
			LogFactory
					.e("user_status changed :",
							"cid:"
									+ statusChangeInPac.getCid()
									+ ",uid :"
									+ statusChangeInPac.getUid()
									+ ",status :"
									+ Integer.toHexString(statusChangeInPac
											.getStatus()));
			removePacketFromTimeoutQueue(statusChangeInPac.get_header_seq());

			// [接到状态改变后发送ACK包]
			if (statusChangeInPac.getServer_msg_id() != 0) {
				ByteBuffer statusACKBuf = StatusAckOutPacket
						.GenerateStatusAckBody(statusChangeInPac
								.getServer_msg_id());
				StatusAckOutPacket statusAckOutPac = new StatusAckOutPacket(
						statusACKBuf, IMOCommand.IMO_STATUS_ACK,
		...
		case IMOCommand.IMO_GET_OFFLINE_MSG_PROFILE:
			GetOfflineMsgProfileInPacket offMsgProfileInPac = new GetOfflineMsgProfileInPacket(
					buf, length);
			offMsgProfileInPac
					.setCommand(IMOCommand.IMO_GET_OFFLINE_MSG_PROFILE);
		...
		case IMOCommand.IMO_GET_OFFLINE_MSG_CONTENTS:
			GetOffMsgFromContactorInPacket offMsgFromContactorInPac = new GetOffMsgFromContactorInPacket(
					buf, length);
			offMsgFromContactorInPac
					.setCommand(IMOCommand.IMO_GET_OFFLINE_MSG_CONTENTS);
			removePacketFromTimeoutQueue(offMsgFromContactorInPac
					.get_header_seq());

			for (int i = 0; i < offMsgFromContactorInPac.getOfflineMsgArray().length; i++) {
				LogFactory
						.e("OffContents",
								""
										+ offMsgFromContactorInPac
												.getOfflineMsgArray()[i]
												.getMsg());
			}

			return offMsgFromContactorInPac;
		case IMOCommand.IMO_DELETE_OFFLINE_MSG:
			DeleteOfflineMsgInPacket deleteMsgInPac = new DeleteOfflineMsgInPacket(
					buf, length);
		...
		case IMOCommand.IMO_OUTER_CONTACTOR_LIST_UC:
			ContactorGroupUCInPacket groupUCInPac = new ContactorGroupUCInPacket(
					buf, length);
		...
		case IMOCommand.IMO_INNER_CONTACTOR_LIST:
			InnerContactorListInPacket innerlistInPac = new InnerContactorListInPacket(
					buf, length);
		...
		case IMOCommand.IMO_OUTER_CONTACTOR_LIST:
			OuterContactorListInPacket outerlistInPac = new OuterContactorListInPacket(
					buf, length);
		...
		case IMOCommand.IMO_OUTER_BASIC_INFO:
			OuterBasicInfoInPacket outerbasicinfoInPac = new OuterBasicInfoInPacket(
					buf, length);
		...
		case IMOCommand.IMO_SERVER_PROMPT:
			ServerPromptInPacket serverPromptInPac = new ServerPromptInPacket(
					buf, length);
			serverPromptInPac.setCommand(IMOCommand.IMO_SERVER_PROMPT);
			return serverPromptInPac;
		case IMOCommand.IMO_LOGIN_QGROUP:
			LoginQGroupInPacket loginGroupInPacket = new LoginQGroupInPacket(
					buf, length);
		...
		case IMOCommand.IMO_GET_USER_QGROUP_LIST:
			GetUserQGroupListInPacket getUserQGroupListInPacket = new GetUserQGroupListInPacket(
					buf, length);
			getUserQGroupListInPacket
					.setCommand(IMOCommand.IMO_GET_USER_QGROUP_LIST);
			return getUserQGroupListInPacket;
		case IMOCommand.IMO_GET_QGROUP_INFO:
			GetQGroupInfoInPacket getQGroupInfoInPacket = new GetQGroupInfoInPacket(
					buf, length);
		...
		case IMOCommand.IMO_GET_QGROUP_USER_LIST:
			GetQGroupUserListInPacket getQGroupUserListInPacket = new GetQGroupUserListInPacket(
					buf, length);
			getQGroupUserListInPacket
					.setCommand(IMOCommand.IMO_GET_QGROUP_USER_LIST);
			return getQGroupUserListInPacket;
		case IMOCommand.IMO_GET_USER_NGROUP_LIST_UC:
			GetUserNgGroupListUcInPacket getUserNgGroupListUcInPacket = new GetUserNgGroupListUcInPacket(
					buf, length);
			getUserNgGroupListUcInPacket
					.setCommand(IMOCommand.IMO_GET_QGROUP_USER_LIST);
			return getUserNgGroupListUcInPacket;
		case IMOCommand.IMO_GET_USER_NGROUP_LIST_UC_NEW:
			GetUserNgGroupListUcNewInPacket getUserNgGroupListUcNewInPacket = new GetUserNgGroupListUcNewInPacket(
					buf, length);
			getUserNgGroupListUcNewInPacket
					.setCommand(IMOCommand.IMO_GET_USER_NGROUP_LIST_UC_NEW);
			return getUserNgGroupListUcNewInPacket;
		case IMOCommand.IMO_CLIENT_LOGIN_NGROUPD:
			GetClientLoginNgroupdInPacket getClientLoginNgroupdInPacket = new GetClientLoginNgroupdInPacket(
					buf, length);
			getClientLoginNgroupdInPacket
					.setCommand(IMOCommand.IMO_CLIENT_LOGIN_NGROUPD);
			return getClientLoginNgroupdInPacket;
		case IMOCommand.IMO_AGREE_JOIN_NGROUP:
			GetAgreeJoinNgroupInPacket getAgreeJoinNgroupInPacket = new GetAgreeJoinNgroupInPacket(
					buf, length);
			getAgreeJoinNgroupInPacket
					.setCommand(IMOCommand.IMO_AGREE_JOIN_NGROUP);
			return getAgreeJoinNgroupInPacket;
		case IMOCommand.IMO_NOTICE_USER_JOIN_NGROUP:
			GetNoticeUserJoinNgroupInPacket getNoticeUserJoinNgroupInPacket = new GetNoticeUserJoinNgroupInPacket(
					buf, length);
			getNoticeUserJoinNgroupInPacket
					.setCommand(IMOCommand.IMO_NOTICE_USER_JOIN_NGROUP);
			return getNoticeUserJoinNgroupInPacket;
		case IMOCommand.IMO_NGROUP_MODIFY_ANNOUNCEMENT_NOTICE:
			GetNgroupModifyAnnouncementNoticeInPacket getNgroupModifyAnnouncementNoticeInPacket = new GetNgroupModifyAnnouncementNoticeInPacket(
					buf, length);
			getNgroupModifyAnnouncementNoticeInPacket
					.setCommand(IMOCommand.IMO_NGROUP_MODIFY_ANNOUNCEMENT_NOTICE);
			return getNgroupModifyAnnouncementNoticeInPacket;
		case IMOCommand.IMO_GET_NGROUP_INFO:
			GetNgroupInfoInPacket getNgroupInfoInPacket = new GetNgroupInfoInPacket(
					buf, length);
		...
		case IMOCommand.IMO_GET_USER_NGROUP_LIST:
			GetUserNgroupListInPacket getUserNgroupListInPacket = new GetUserNgroupListInPacket(
					buf, length);
			getUserNgroupListInPacket
					.setCommand(IMOCommand.IMO_GET_USER_NGROUP_LIST);
			return getUserNgroupListInPacket;
		case IMOCommand.IMO_GET_NGROUP_USER_LIST:
			GetNgroupUserListInPacket getNgroupUserListInPacket = new GetNgroupUserListInPacket(
					buf, length);
			getNgroupUserListInPacket
					.setCommand(IMOCommand.IMO_GET_NGROUP_USER_LIST);
			return getNgroupUserListInPacket;
		case IMOCommand.IMO_GET_NGROUP_USERS_STATUS:
			GetNgroupUsersStatusInPacket getNgroupUsersStatusInPacket = new GetNgroupUsersStatusInPacket(
					buf, length);
			getNgroupUsersStatusInPacket
					.setCommand(IMOCommand.IMO_GET_NGROUP_USERS_STATUS);
			return getNgroupUsersStatusInPacket;
		case IMOCommand.IMO_NGOURP_CHAT:
			GetNgroupChatInPacket getNgroupChatInPacket = new GetNgroupChatInPacket(
					buf, length);
		...
		case IMOCommand.IMO_NEW_MSG_NOTICE_NGROUP:
			GetNewMsgNoticeNgroupInPacket getNewMsgNoticeNgroupInPacket = new GetNewMsgNoticeNgroupInPacket(
					buf, length);
			getNewMsgNoticeNgroupInPacket
					.setCommand(IMOCommand.IMO_NEW_MSG_NOTICE_NGROUP);
			return getNewMsgNoticeNgroupInPacket;
		case IMOCommand.IMO_NGROUP_USER_CHANGE_STATUS_NOTICE:
			GetNgroupUserChangeStatusNoticeInPacket getNgroupUserChangeStatusNoticeInPacket = new GetNgroupUserChangeStatusNoticeInPacket(
					buf, length);
			getNgroupUserChangeStatusNoticeInPacket
					.setCommand(IMOCommand.IMO_NGROUP_USER_CHANGE_STATUS_NOTICE);
			return getNgroupUserChangeStatusNoticeInPacket;
		case IMOCommand.IMO_EXIT_NGROUP:
			GetExitNgroupInPacket getExitNgroupInPacket = new GetExitNgroupInPacket(
					buf, length);
		...
		case IMOCommand.IMO_EXIT_NGROUP_NOTICE_USERS:
			GetExitNgroupNoticeUsersInPacket getExitNgroupNoticeUsersInPacket = new GetExitNgroupNoticeUsersInPacket(
					buf, length);
			getExitNgroupNoticeUsersInPacket
					.setCommand(IMOCommand.IMO_EXIT_NGROUP_NOTICE_USERS);
			return getExitNgroupNoticeUsersInPacket;
		case IMOCommand.IMO_MODIFY_NGROUP_NAME_NOTICE:
			GetModifyNgroupNameNoticeInPacket getModifyNgroupNameNoticeInPacket = new GetModifyNgroupNameNoticeInPacket(
					buf, length);
			getModifyNgroupNameNoticeInPacket
					.setCommand(IMOCommand.IMO_MODIFY_NGROUP_NAME_NOTICE);
			return getModifyNgroupNameNoticeInPacket;
		case IMOCommand.IMO_NGROUP_KICK_USER_NOTICE:
			GetNgroupKickUserNoticeInPacket getNgroupKickUserNoticeInPacket = new GetNgroupKickUserNoticeInPacket(
					buf, length);
			getNgroupKickUserNoticeInPacket
					.setCommand(IMOCommand.IMO_NGROUP_KICK_USER_NOTICE);
			return getNgroupKickUserNoticeInPacket;
		case IMOCommand.IMO_GET_OFFLINE_NGROUP_CHAT_MSG:
			GetOffLineNgroupChatMsgInPacket getOffLineNgroupChatMsgInPacket = new GetOffLineNgroupChatMsgInPacket(
					buf, length);
			getOffLineNgroupChatMsgInPacket
					.setCommand(IMOCommand.IMO_GET_OFFLINE_NGROUP_CHAT_MSG);
			return getOffLineNgroupChatMsgInPacket;
		case IMOCommand.IMO_NGROUP_NOTICE:
			GetNgroupNoticeInPacket getNgroupNoticeInPacket = new GetNgroupNoticeInPacket(
					buf, length);
		...
		case IMOCommand.IMO_NEW_MSG_NOTICE_QGROUP:
			NewMsgNoticeQGroupInPacket newMsgNoticeQGroupInPacket = new NewMsgNoticeQGroupInPacket(
					buf, length);
			newMsgNoticeQGroupInPacket
					.setCommand(IMOCommand.IMO_NEW_MSG_NOTICE_QGROUP);
			return newMsgNoticeQGroupInPacket;
		case IMOCommand.IMO_QGROUP_USER_CHANGE_STATUS_NOTICE:
			QGroupUserChangeStatusNoticeInPacket qGroupUserChangeStatusNoticeInPacket = new QGroupUserChangeStatusNoticeInPacket(
					buf, length);
			qGroupUserChangeStatusNoticeInPacket
					.setCommand(IMOCommand.IMO_QGROUP_USER_CHANGE_STATUS_NOTICE);
		...
		case IMOCommand.IMO_EXIT_QGROUP_NOTICE_USERS:
			ExitQgroupNoticeUsersInPacket exitQgroupNoticeUsersInPacket = new ExitQgroupNoticeUsersInPacket(
					buf, length);
			exitQgroupNoticeUsersInPacket
					.setCommand(IMOCommand.IMO_EXIT_QGROUP_NOTICE_USERS);
			return exitQgroupNoticeUsersInPacket;
		case IMOCommand.IMO_MODIFY_QGROUP_NAME_NOTICE:
			ModifyQgroupNameNoticeInPacket modifyQgroupNameNoticeInPacket = new ModifyQgroupNameNoticeInPacket(
					buf, length);
			modifyQgroupNameNoticeInPacket
					.setCommand(IMOCommand.IMO_MODIFY_QGROUP_NAME_NOTICE);
			return modifyQgroupNameNoticeInPacket;
		case IMOCommand.IMO_DESTORY_THE_QGROUP:
			DestoryTheQgroupInPacket destoryTheQgroupInPacket = new DestoryTheQgroupInPacket(
					buf, length);
			destoryTheQgroupInPacket
					.setCommand(IMOCommand.IMO_DESTORY_THE_QGROUP);
			return destoryTheQgroupInPacket;
		case IMOCommand.IMO_QGROUP_KICK_USER_NOTICE:
			QGroupKickUserNoticeInPacket qcGroupKickUserNoticeInPacket = new QGroupKickUserNoticeInPacket(
					buf, length);
			qcGroupKickUserNoticeInPacket
					.setCommand(IMOCommand.IMO_QGROUP_KICK_USER_NOTICE);
			return qcGroupKickUserNoticeInPacket;
		case IMOCommand.IMO_GET_QGROUP_OFFLINE:
			GetQGroupOfflineInPacket getQGroupOfflineInPacket = new GetQGroupOfflineInPacket(
					buf, length);
			getQGroupOfflineInPacket
					.setCommand(IMOCommand.IMO_GET_QGROUP_OFFLINE);
			return getQGroupOfflineInPacket;
		case IMOCommand.IMO_GET_OFFLINE_QGROUP_CHAT_MSG:
			GetOfflineQGroupChatMsgInPacket getOfflineQGroupChatMsgInPacket = new GetOfflineQGroupChatMsgInPacket(
					buf, length);
			getOfflineQGroupChatMsgInPacket
					.setCommand(IMOCommand.IMO_GET_OFFLINE_QGROUP_CHAT_MSG);
			return getOfflineQGroupChatMsgInPacket;
		case IMOCommand.IMO_DELETE_QGROUPD_OFFLINE:
			DeleteQGroupOfflineInPacket deleteQGroupOfflineInPacket = new DeleteQGroupOfflineInPacket(
					buf, length);
			deleteQGroupOfflineInPacket
					.setCommand(IMOCommand.IMO_DELETE_QGROUPD_OFFLINE);
			return deleteQGroupOfflineInPacket;
		case IMOCommand.IMO_QGROUP_DESTROY_NOTICE_USER:
			QgroupDestroyNoticeUserInPacket qgroupDestroyNoticeUserInPacket = new QgroupDestroyNoticeUserInPacket(
					buf, length);
			qgroupDestroyNoticeUserInPacket
					.setCommand(IMOCommand.IMO_QGROUP_DESTROY_NOTICE_USER);
