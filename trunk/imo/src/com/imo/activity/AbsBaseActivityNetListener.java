 ...
 * 
 */
public abstract class AbsBaseActivityNetListener extends AbsBaseActivity
		implements PacketsObserver {
	...
	protected NIOThread mNIOThread = AppService.getService() != null ? AppService
			.getService().getNIOThreadInstance() : null;

	// private TCPConnection tcpConnection = getTcpConnection();

	protected TCPConnection getTcpConnection() {
		return AppService.getService() != null ? AppService.getService()
				.getTcpConnection() : null;
	}
	...
	protected void resetConnection() {
		...
		LogFactory.e("AbsBaseActivity", "isStartRelogin :"
				+ EngineConst.isStartRelogin);
	}

	static {
		if (AppService.getService() == null
				|| AppService.getService().getCurNIOThreadInstance() == null) {
	...
	public void NotifyPacketArrived(String aConnectionId, short command) {
		...
		case IMOCommand.IMO_LOGIN: {
			...
			if (this instanceof LoginActivity
				...
				if (IMOApp.getApp().mLastActivity instanceof OrganizeActivity) {
					if (OrganizeActivity.getActivity() != null) {
						OrganizeActivity.getActivity().titleBarHandler
								.sendMessage(msg);
					}
				} else if (IMOApp.getApp().mLastActivity instanceof ContactActivity) {
					if (ContactActivity.getActivity() != null) {
						ContactActivity.getActivity().titleBarHandler
								.sendMessage(msg);
					}
				}
			}
	 ...
	 * 
	 */
	protected void doEmployeeState(int aContactorsNum,
	...
	private void reLogin(short command) {
		...
		if (ret == 17) {// 重新登录
			String loginName = (String) PreferenceManager.get(Globe.SP_FILE,
					new String[] { LoginActivity.LOGIN_NAME, new String() });
			String loginPwd = (String) PreferenceManager.get(Globe.SP_FILE,
					new String[] { LoginActivity.LOGIN_PWD, new String() });
	...
	public void requestLogOut(boolean isExitApp) {
		...
		LogFactory.d("AbsBase",
				"-------->requestLogOut  ............isExitApp " + isExitApp);
	...
	public void NotifyPacketProgress(String aConnectionId, short command,
		...
		if (EngineConst.IMO_CONNECTION_ID.equals(aConnectionId))
			...
			case IMOCommand.ERROR_REMOTE_DATA:
				LogFactory.e("ERROR_REMOTE_DATA",
						"ERROR_REMOTE_DATA------------------->");
	...
	public void responseLogout(short aTotalLen, short aSendedLen) {
		...
		if (aTotalLen == aSendedLen) {
			...
			LogFactory.d("AbsBase", "-------->Logout  ............isExitApp "
					+ mGlobal.isExitApp);
	...
	private void getChatMessage(short command) {
		SendMsgInPacket sendMsgInPacket = (SendMsgInPacket) IMOApp
		...
		if (sendMsgInPacket != null) {
			...
			// 存放到数据库
			try {
				MessageInfo msg = new MessageInfo(0, fromUid, "",
						EngineConst.uId, "", Functions.getDate(),
						Functions.getTime(), message,
						MessageInfo.MessageInfo_From,
						MessageInfo.MessageInfo_MsgId, isRead,
						MessageInfo.MessageInfo_UnFailed);
	...
	private void updateRecentMap(int fromCid, int fromUid, String name,
			String message, String time, int isRead) {
		LogFactory.d("xxx", "updateRecentMap in the DialogueActivity...");

		RecentContactInfo info = new RecentContactInfo(fromCid, fromUid, name,
				message, time, isRead == 0 ? 1 : 0,
		...
		try {
			...
			// RecentContactInfo.NORMAL_TYPE);
			IMOApp.imoStorage.updateRecentContact(info,
					info.getTime().split(" ")[0]);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private void sendACK(int fromCid, int fromUid, int server_msg_id) {
		ByteBuffer aBody = SendMsgAckOutPacket.GenerateMsgTextAckBody(fromCid,
				fromUid, server_msg_id);
		SendMsgAckOutPacket sendMsgAckOutPacket = new SendMsgAckOutPacket(
				aBody, IMOCommand.IMO_SEND_MESSAGE_ACK, EngineConst.cId,
				EngineConst.uId);
		mNIOThread.send(EngineConst.IMO_CONNECTION_ID, sendMsgAckOutPacket,
				false);
	}
		...
		mParentHandler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				case NotifyPacketProgress: {
					if (mGlobal.mLastActivity instanceof OrganizeActivity
							|| mGlobal.mLastActivity instanceof ContactActivity) {
						if ((!isFinishing()) && hasExecute == false) {
							responseLogout((short) 0, (short) 0);
						}
					}
			...
			private void handleServerPrompt(Message msg) {
				...
				ServerPromptInPacket serverPromptInPacket = (ServerPromptInPacket) IMOApp
						.getDataEngine().getInPacketByCommand(command);
			...
			private void handleFailed(Message msg) {
				...
				absDialog = DialogFactory.alertDialog(/* mContext* */getParent(),
						resources.getString(R.string.warn), (String) msg.obj,
	...
	private void handleTimeOut(Message msg) {
		...
		if (command == IMOCommand.IMO_GET_OFFLINE_MSG_PROFILE
				|| command == IMOCommand.IMO_GET_EMPLOYEE_STATUS) {
			// /脱机摘要、获取用户状态不提示网络超时
			return;
		} else if (command == IMOCommand.IMO_GET_EMPLOYEE_PROFILE
				&& !(IMOApp.getApp().mLastActivity instanceof FirstLoadingActivity || IMOApp
						.getApp().mLastActivity instanceof NormalLoadingActivity)) {
			// /获取名片，不提示网络超时
			return;
	...
	public void sendCrashMSG() {
		...
		if (errorinfo != null) {
			...
			ByteBuffer dataBuf = ReportErrorInfoOutPacket
					.GenerateErrorInfoBody(message.toString());
			ReportErrorInfoOutPacket out = new ReportErrorInfoOutPacket(
					dataBuf, IMOCommand.IMO_REPORT_ERROR, EngineConst.cId,
					EngineConst.uId);
	...
	protected void doRequestAppVersion() {
		...
		if (mGlobal.mLastActivity instanceof SystemSetActivity) {
			Toast.makeText(mContext, getString(R.string.app_update_loading),
					Toast.LENGTH_SHORT).show();
		}
		...
		ByteBuffer bufferBody = UpdateVersionOutPacket
				.GenerateUpdateVersionBody(aVersion, aBuild, aDevNo, aDevType);

		UpdateVersionOutPacket outPacket = new UpdateVersionOutPacket(
				bufferBody, IMOCommand.IMO_UPDATE_VERSION, EngineConst.cId,
				EngineConst.uId);
	...
	protected void responseAppVersion(short command) {
		...
		UpdateVersionInPacket inPacket = (UpdateVersionInPacket) IMOApp
				.getDataEngine().getInPacketByCommand(command);
	...
	public Handler mUpdateAppHandler = new Handler() {
		@Override
		public void handleMessage(Message msg) {
			LogFactory.d("xxUpdate",
					"mUpdateAppHandler-------------> handler message");
