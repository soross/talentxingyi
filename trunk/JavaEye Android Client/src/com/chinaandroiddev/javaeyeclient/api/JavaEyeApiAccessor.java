...
public class JavaEyeApiAccessor {
	...
	public static VerifiedInfo verify() throws Exception {
		...
		try {
			jsonString = BaseAuthenicationHttpClient.doRequest(url, user.name,
					user.password);
		} catch (JEException e) {// 如果登录信息不正确的话,
									// 会出现这样的exception:"received authentication challenge is null"
			if (e.getMessage()
					.startsWith(
	...
	// twitter
	public static ArrayList<Update> getFollowed(long lastId, int pageNo)
			throws Exception {
		String url = "http://api.javaeye.com/api/twitters/list";
		return getUpdatesList(url, lastId, pageNo);
	}

	public static ArrayList<Update> getReplies(long lastId, int pageNo)
			throws Exception {
		String url = "http://api.javaeye.com/api/twitters/replies";
		return getUpdatesList(url, lastId, pageNo);
	}

	public static ArrayList<Update> getAllUpdates(long lastId, int pageNo)
			throws Exception {
		String url = "http://api.javaeye.com/api/twitters/all";
		return getUpdatesList(url, lastId, pageNo);
	}

	private static ArrayList<Update> getUpdatesList(String url, long lastId,
	...
	public static boolean createUpdate(Update update) throws Exception {
		...
		// StringBuffer("http://api.javaeye.com/api/twitters/create?");
		...
		String temp = BaseAuthenicationHttpClient.doRequest(url.toString(),
				user.name, user.password, map);
	...
	public static boolean deleteUpdate(Update update) throws Exception {
		String url = new String(
				"http://api.javaeye.com/api/twitters/destroy?id=" + update.id);
		String temp = BaseAuthenicationHttpClient.doRequest(url, user.name,
				user.password);
		if (temp.startsWith("error")) {
			return false;
		}
	...
	public static Update getUpdateById(long id) throws Exception {
		String url = new String("http://api.javaeye.com/api/twitters/show?id="
				+ id);
		String jsonString = BaseAuthenicationHttpClient.doRequest(url,
				user.name, user.password);
	...
	public static ArrayList<Update> getUpdatesByIds(long[] ids)
			throws Exception {
		ArrayList<Update> ret = new ArrayList<Update>();
		...
		String jsonString = BaseAuthenicationHttpClient.doRequest(
				url.toString(), user.name, user.password, map);
		if (jsonString.startsWith("error")) {
			return ret;
		}
	...
	private static FavoriteItem jsonToFavorite(JSONObject json)
	...
	public static FavoriteItem addFavorite(FavoriteItem item) throws Exception {
		...
		String jsonString = BaseAuthenicationHttpClient.doRequest(url,
				user.name, user.password, params);

		return jsonToFavorite(new JSONObject(jsonString));
	}

	public static FavoriteItem updateFavorite(FavoriteItem item)
			throws Exception {
		...
		String jsonString = BaseAuthenicationHttpClient.doRequest(
				url.toString(), user.name, user.password, params);
		if (jsonString.startsWith("error")) {
			return null;
		}
		return jsonToFavorite(new JSONObject(jsonString));
	...
	public static boolean deleteFavorite(FavoriteItem item) throws Exception {
		String url = "http://api.javaeye.com/api/user_favorites/destroy?id="
				+ item.id;
		String jsonString = BaseAuthenicationHttpClient.doRequest(url,
				user.name, user.password);
		if (jsonString.startsWith("error")) {
	...
	// message start
	public static ArrayList<Message> inBox(long lastId, int page)
			throws Exception {
	...
	public static Message replyMessage(Message message) throws Exception {
		StringBuffer url = new StringBuffer(
				"http://api.javaeye.com/api/messages/reply?");
		HashMap map = new HashMap();
		map.put("title", message.title);
		map.put("body", message.body);
	...
	public static boolean deleteMessage(Message message) throws Exception {
		String url = "http://api.javaeye.com/api/messages/destroy?id="
				+ message.id;
		String jsonString = BaseAuthenicationHttpClient.doRequest(url,
				user.name, user.password);
		if (jsonString.startsWith("error")) {
