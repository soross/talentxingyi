...
public class MessageNew extends Activity {
		...
		msgButtonSend.setOnClickListener(new Button.OnClickListener() {
			...
			public void onClick(View v) {
				if (msgReply.getText().toString().length() <= 0
						|| replyTo.getText().toString().length() <= 0
						|| title.getText().toString().length() <= 0) {
					FlurryAgent.onError("New Message Error",
							"to/subject/body cannot be empty", LOG_TAG);
					new AlertDialog.Builder(MessageNew.this).setTitle("警告")
	...
	private void sendMsg() {
		progressDialog = ProgressDialog.show(MessageNew.this, "请稍等...",
				"短信发送中...", true);
		new Thread() {
			public void run() {
				...
				try {
					...
					FlurryAgent.onError("Send Message Error", e.getMessage(),
							LOG_TAG);
					m = null;
				}
				if (m == null) {
