...
public class RecentContactActivity extends AbsBaseActivityNetListener {
	...
	OnItemClickListener onItemClickListener = new OnItemClickListener() {
		...
		private void goToDialogue(Bundle data, RecentContactInfo info) {
			 ...
			 * data.putBoolean("sex"
			 * ,isBoy(getEmployeeInfoByUid(info.getUid()).getGender())); }else
			 * if (mGlobal.outerContactInfoMap.get(info.getUid())!=null) {
			 * data.putBoolean
			 * ("sex",isBoy(mGlobal.outerContactInfoMap.get(info.getUid
			 * ()).getGender())); }else{ LogFactory.d(TAG, "error data"); }
			 */
	...
	// ====================================================
	/**
	 * 1-请求：脱机消息 旧版本的 获取所有的脱机消息
	 */
	/*
	 * private void doRequestOfflineMSG() { LogFactory.d(TAG ,
	 * "doRequestOfflineMSG "); offlineMsgItemList.clear(); CommonOutPacket
	 * outPacket = new CommonOutPacket(ByteBuffer.allocate(0),
	 * IMOCommand.IMO_GET_OFFLINE_MSG, EngineConst.cId, EngineConst.uId); if
	 * (mNIOThread != null) { mNIOThread.send(EngineConst.IMO_CONNECTION_ID,
	 * outPacket, false); tcpConnection.addToObserverList(this); } }
	 */
	...
	// ArrayList<OfflineMsgItem> offlineMsgItemList = new
	...
	private void responseOfflineMSG(short command) {/*
													 ...
													 * int num; //
													 * OfflineMsgItem[]
													 * offlineMsgArray = null;
													 * LogFactory.d(TAG,
													 * "------------responseOfflineMSG----->"
													 * ); GetOfflineMsgInPacket
													 * inPacket
													 * =(GetOfflineMsgInPacket)
													 * tcpConnection
													 * .getInPacketByCommand
													 * (command); short
													 * commandRet =
													 * inPacket.getRet(); byte
													 * endflag =
													 * inPacket.getEndflag(); if
													 * (commandRet == 0) {
													 * LogFactory.d(TAG,
													 * "------------endflag--> "
													 * + endflag);
													 * OfflineMsgItem[]
													 * temp_offlineMsgArray =
													 * inPacket
													 * .getOfflineMsgArray();
													 * for (int i = 0; i <
													 * temp_offlineMsgArray
													 * .length; i++) {
													 * offlineMsgItemList
													 * .add(temp_offlineMsgArray
													 * [i]);
													 * LogFactory.d(TAG+"OfflineMSG"
													 * ,
													 * temp_offlineMsgArray[i].
													 * toString()); } } if
													 * (endflag == 1) { /// Hava
													 * no OfflineMSG if
													 * (offlineMsgItemList
													 ...
													 * ); }else {
													 * LogFactory.d(TAG,
													 * "------------endflag--> OfflineMSG hava got All !! \t count ="
													 * +
													 * offlineMsgItemList.size()
													 * ); if
													 * (offlineMsgItemList.
													 * size() >= 1) {
													 * OfflineMsgItem item =
													 * offlineMsgItemList
													 * .get(offlineMsgItemList
													 * .size() - 1); // DB Save
													 * Success if (doSave2DB())
													 * {
													 * ///查询数据库得到，需要删除的多个联系人的脱机消息
													 * // mGlobal.imoStorage.xx(
													 * delOfflineMsgIdList);
													 * //请求删除 curPos = 0; if
													 * (delOfflineMsgIdList !=
													 * null &&
	...
	private void mergeData(ArrayList<OfflineMsgProfileItem> offlineMsgProfileItemList) {
		...
		for (int i = 0; i < offlineMsgProfileItemList.size(); i++) {
			...
			if (id_time_Map.get(info.getUid()) != null) { // //has exist ,need
															// del map
				RecentContactInfo recentContactInfo = delSameInDifferentCategory(info);
				// int oldCount =
				// recentContactInfo!=null?recentContactInfo.getCount():0;
			...
			// recentContactMap = new HashMap<String,
			// HashMap<Integer,RecentContactInfo>>();
		}
	...
	// }
	// ;
	// // IMOApp.imoStorage.addMessage(new MessageInfo(0,
	// offlineMsgItem.getFromcid(), "", EngineConst.uId,
	...
	// if(count > 0){
	// ((MainActivityGroup)
	// RecentContactActivity.this.getParent()).update2ShowNew(true);
	// }else{
	// ((MainActivityGroup)
	// RecentContactActivity.this.getParent()).update2ShowNew(true);
	// }
	...
	public void addOrUpdate(RecentContactInfo info, Boolean isFromDialogue) {
		...
		if (id_time_Map.get(info.getUid()) != null) { // //has exist ,need del
														// map
			if (!isFromDialogue) {
				RecentContactInfo recentContactInfo = delSameInDifferentCategory(info);
		...
		// / add userInfo in defferent category
		// recentContactMap.get(info.getTime().split(" ")[0]).put(info.getUid(),
		// info);
		String date = info.getTime().split(" ")[0];
		HashMap<Integer, RecentContactInfo> idMap = null;
		if (recentContactMap.get(date) != null) {
			...
			if (Globe.is_notification) {
				...
				try {
					if (info.getInfo() != null && !info.getInfo().equals("")) {
						...
						// showLastMessage =
						// MessageDataFilter.jsonToCharSequence(new JSONObject(
						// info.getInfo()));
					} else {
						showLastMessage = "";
					}
	...
	private void buildShowItemInfo() {
		...
		for (int j = 0; j < idList.size(); j++) {
			...
			for (int k = 0; k < dateList.size(); k++) {
				...
				if (recentContactInfo != null) {
					// id_time_list.add(new
					// Object[]{idList.get(i),recentContactInfo.getTime()});
	...
	private void getRecentContactData(ArrayList<String> dateList, ArrayList<Integer> idList, HashMap<String, HashMap<Integer, RecentContactInfo>> recentContactMap) {
	...
	private void responseServerPullEmployeeState(short command) {
		...
		// LogFactory.d("pull", "mGlobal.userStateMap.get(uid)== null :"
		// +(mGlobal.userStateMap.get(uid)== null));
		// LogFactory.d("pull", "mGlobal.outerUserStateMap.get(uid)== null :"
		// +(mGlobal.outerUserStateMap.get(uid)== null));
