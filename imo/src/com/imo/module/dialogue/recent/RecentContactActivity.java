...
public class RecentContactActivity extends AbsBaseActivityNetListener {
	...
	/**标示已经获得了脱机消息摘要*/
//	private boolean hasGetOfflineMSGProfile = false;
			...
			public void onItemClick(AdapterView<?> adapterView, View view,
				...
				if (info != null && info.getCid()!=-1) {
					...
					if (info.getCount()-getMsgCount >0 ) {
						if (mGlobal.sendMsgUserId.contains(info.getUid())) {
							mGlobal.sendMsgUserId.remove(info.getUid());
						}
					}
...
//					if (info.hasOfflineMSG()) {
//						info.setHasOfflineMSG(false);////
//						dialog.show();////加载脱机消息，提示进度条
//						beginLoading(TYPE_OFFLINEMSG_SINGLE);
//						doRequestSignleOfflineMSG(info.getCid(), info.getUid());
//					}else{
...
/*				if(getEmployeeInfoByUid(info.getUid())!=null){
					data.putBoolean("sex",isBoy(getEmployeeInfoByUid(info.getUid()).getGender()));
				}else if (mGlobal.outerContactInfoMap.get(info.getUid())!=null) {
					data.putBoolean("sex",isBoy(mGlobal.outerContactInfoMap.get(info.getUid()).getGender()));
				}else{
					LogFactory.d(TAG, "error data");
				}*/
	...
	private void updateTitleBar() {
		...
		if (curIsEditState) {
			 mTitleBar.initDefaultTitleBar(
					 	resources.getString(R.string.dialgue_title),
...
//	====================================================
	 ...
	 * 旧版本的 获取所有的脱机消息
	 */
/*	private void doRequestOfflineMSG() {
    	
    	LogFactory.d(TAG , "doRequestOfflineMSG ");
    	
    	offlineMsgItemList.clear();
    	
    	CommonOutPacket outPacket = new CommonOutPacket(ByteBuffer.allocate(0), IMOCommand.IMO_GET_OFFLINE_MSG,  EngineConst.cId, EngineConst.uId);
    	if (mNIOThread != null) {
	...
	private void doRefresh() {
		if (EngineConst.isNetworkValid
				&& AppService.getService().getTcpConnection() != null
				&& AppService.getService().getTcpConnection().isConnected()) {
...
//	/**
//	 * 执行删除操作:
//	 * 删除某人（fromcid，fromuid）发的id之前的脱机聊天消息
//	 */
//	private void doDelete(int aFromid){
//		doRequestDelRemoteOfflineMSG(EngineConst.cId, EngineConst.uId, aFromid);
//	}
...
//				IMOCommand.IMO_GET_OFFLINE_MSG_CONTENTS == command ||
							...
							IMOCommand.IMO_GET_OFFLINE_MSG_PROFILE == command ||
							IMOCommand.IMO_UPDATE_USER_STATUS== command
				)
			return true;
...
//		int tnum;
//		int num;
//		OfflineMsgItem[] offlineMsgArray = null;
		
		LogFactory.d(TAG, "------------responseOfflineMSG----->");
		
		GetOfflineMsgInPacket inPacket =(GetOfflineMsgInPacket) tcpConnection.getInPacketByCommand(command);
		...
		if (commandRet == 0) {
			...
			LogFactory.d(TAG, "------------endflag--> " + endflag);
			...
			for (int i = 0; i < temp_offlineMsgArray.length; i++) {
				...
				LogFactory.d(TAG+"OfflineMSG", temp_offlineMsgArray[i].toString());
			}
		}
		...
		if (endflag == 1) {
					...
					/// Hava no OfflineMSG  
				if (offlineMsgItemList.size() ==0) {
					refresh(null);
					LogFactory.d(TAG, "-------------> Have no OfflineMSG!!");
				}else {
					LogFactory.d(TAG, "------------endflag--> OfflineMSG hava got All !! \t count =" + offlineMsgItemList.size() );
					if (offlineMsgItemList.size() >= 1) {
	
						OfflineMsgItem item = offlineMsgItemList.get(offlineMsgItemList.size() - 1);
	
					//  DB Save Success 
						if (doSave2DB()) {
							///查询数据库得到，需要删除的多个联系人的脱机消息
	//						mGlobal.imoStorage.xx(delOfflineMsgIdList);
							
								//请求删除
								curPos = 0;
								if (delOfflineMsgIdList != null && delOfflineMsgIdList.size() != 0) {
									curDelIdArray = delOfflineMsgIdList.get(curPos);
									//	请求删除服务器端
									 doRequestDelRemoteOfflineMSG(curDelIdArray[0],curDelIdArray[1], curDelIdArray[2]);
								}
	
								// Test for del=============================begin
	//							LogFactory.d(TAG, "Msgid = " + item.getMsgid());
	//							doRequestDelRemoteOfflineMSG(item.getFromcid(),item.getFromuid(), item.getMsgid());
								// Test for del=============================end							}
					}
				}
		}
...
//		 short ret;
//		 byte endflag;
//		 OfflineMsgProfileItem[] offlineMsgArray;
		...
		if (commandRet == 0) {
			...
			for (int i = 0; i < temp_offlineMsgProfileArray.length; i++) {
				...
				//lee 更新所有发送过用户的远程id
				if (!mGlobal.sendMsgUserId.contains(temp_Item.getFromuid())) {
					mGlobal.sendMsgUserId.add(temp_Item.getFromuid());
				}
			}
		}
...
//			Node tempNode = getNodeByUid(item.getFromuid());
//			String name = tempNode!=null ?tempNode.getNodeData().nodeName : "";
			info = new RecentContactInfo(
					item.getFromcid(), 
					item.getFromuid(), 
					"",///name,getView的时候获取
//					getEmployeeInfoByUid(item.getFromuid())!=null?getEmployeeInfoByUid(item.getFromuid()).getName():"",///name 
					item.getLastmsg(), 
					formatServerTime(item.getTime()), 
					item.getCount(),
			...
			if (id_time_Map.get(info.getUid())!= null) { ////has exist ,need del map
...
//				info.setCount(oldCount + info.getCount());
			...
			/// add userInfo in defferent category
...
//			recentContactMap = new HashMap<String, HashMap<Integer,RecentContactInfo>>();
	...
	/**
	 * del the same in the different category .
	 * 
	 * @param info
	 * @return
	 */
//	private int delSameInDifferentCategory(RecentContactInfo info) {
//		
...
//		}
//		
//		return recentContactInfo!=null?recentContactInfo.getCount():0;
//	}
...
//		updateShowList(false);
	...
	public void deleteRecentUser(int uid){
		...
		boolean result = true;
		try {
			result = IMOApp.getApp().imoStorage.deleteRecentContact(RecentContactInfo.NORMAL_TYPE, uid);
		} catch (Exception e) {
	...
	//	============================
	private Integer[] curDelIdArray = null;
	private ArrayList<Integer[]> delOfflineMsgIdList = null ;
	private int curPos = 0;

	private int curFromUid = 1;
	private int curFromCid = 0;
...
//		int msgId =  -1;
//		String msg =  null;
//		OfflineMsgItem offlineMsgItem = null;
//		for (int i = 0; i < offlineMsgItemList.size(); i++) {
//			offlineMsgItem = offlineMsgItemList.get(i);
////			private int fromcid;
////			private int fromuid;
////			private int time;
////			private int msgid;
...
//			try {
//				show_message = MessageDataFilter.jsonToCharSequence(new JSONObject(msg));
//				
//			} catch (JSONException e1) {
//				e1.printStackTrace();
//			}
//			;
////			IMOApp.imoStorage.addMessage(new MessageInfo(0, offlineMsgItem.getFromcid(), "", EngineConst.uId,
////					"", Functions.getDate(), time, message, 0));
//			
//		} catch (Exception e) {
//			e.printStackTrace();
//		}
...
//			if(count > 0){
//				((MainActivityGroup) RecentContactActivity.this.getParent()).update2ShowNew(true);
//			}else{
//				((MainActivityGroup) RecentContactActivity.this.getParent()).update2ShowNew(true);
//			}
//	}
...
//	private ArrayList<Object[]> id_time_list = null;
...
//	================================
	...
	private void loadRecentContactData() {
		...
		try {
			mGlobal.imoStorage.getRecentContactData(dateList,idList,recentContactMap);
...
//	private boolean dateNeedSort = false;
	/**
	 * 更新显示列表:根据情况需要进行排序
	 */
...
//		updateShowView();
	...
	public  void addOrUpdate(RecentContactInfo info,Boolean isFromDialogue){
		...
		if (info.getCount() >0) {
			...
			/// add userInfo in defferent category
//			recentContactMap.get(info.getTime().split(" ")[0]).put(info.getUid(), info);
			String date = info.getTime().split(" ")[0];
			HashMap<Integer, RecentContactInfo> idMap = null;
			if(recentContactMap.get(date)!=null){
				idMap = recentContactMap.get(date);
					...
					try {
						if (info.getInfo()!=null && !info.getInfo().equals("")) {
							showLastMessage = MessageDataFilter.jsonToText(new JSONObject(
									info.getInfo()));
//							showLastMessage = MessageDataFilter.jsonToCharSequence(new JSONObject(
//									info.getInfo()));
						}else {
							showLastMessage="";
						}
					...
					if (needNotice) {
						NoticeManager.updateNewsNotice(
								notificationManager, 
								notice, 
								"",
								showLastMessage,
	...
	private void buildShowItemInfo() {
		...
		for (int j = 0; j < idList.size(); j++) {
			...
			for (int k = 0; k < dateList.size(); k++) {
				...
				if (recentContactInfo != null) {
					// id_time_list.add(new  Object[]{idList.get(i),recentContactInfo.getTime()});
	...
	private void getRecentContactData(ArrayList<String> dateList,
	...
	private void sendACK(int fromCid, int fromUid, int server_msg_id) {
		...
		SendMsgAckOutPacket sendMsgAckOutPacket = new SendMsgAckOutPacket(aBody, IMOCommand.IMO_SEND_MESSAGE_ACK,
...
//		LogFactory.d("pull", "mGlobal.userStateMap.get(uid)== null :" +(mGlobal.userStateMap.get(uid)== null));
//		LogFactory.d("pull", "mGlobal.outerUserStateMap.get(uid)== null :" +(mGlobal.outerUserStateMap.get(uid)== null));
...
//			try {
//				mGlobal.imoStorage.addRecentContact(new RecentContactInfo(
//						aboutCid, aboutUid, aboutName, "", Functions.getTime(),
//						RecentContactInfo.NORMAL_TYPE), Functions.getDate());
//			} catch (Exception e) {
//				e.printStackTrace();
//			}
//		}
