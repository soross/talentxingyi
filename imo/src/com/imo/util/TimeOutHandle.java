...
public class TimeOutHandle {
	...
	private void initDialog(final Context mContext) {
		...
		mTimeOutDialog = DialogFactory.alertDialog(mContext, mContext.getString(R.string.warn), mContext.getString(R.string.notifypacket_timeout), new String[] {
			IMOApp.getApp().getString(R.string.ok)
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				// }else if(IMOApp.getApp().mLastActivity instanceof
				// FirstLoadingActivity) {
				// EngineConst.isLoginSuccess = false;
				// IMOApp.getApp().turn2LoginForLogout();
				// } else if(IMOApp.getApp().mLastActivity instanceof
				// NormalLoadingActivity) {
				// EngineConst.isLoginSuccess = false;
				// IMOApp.getApp().turn2LoginForLogout();
				// // } else if(IMOApp.getApp().mLastActivity instanceof
				// WorkSignActivity) {
	...
	public void showDialog(Context mContext) {
		...
		/*
		 * ///重登陆超时，直接跳到登陆界面 if (IMOApp.getApp().reLoginTimeOut) {
		 * ////再次打开失败接受大门 IMOApp.getApp().hasPackageFailed = false; if
		 * (!(IMOApp.getApp().mLastActivity instanceof LoginActivity)) {
		 * Toast.makeText(mContext, "网络异常，请您重新登录。", Toast.LENGTH_SHORT).show();
		 * EngineConst.isLoginSuccess = false;
		 * IMOApp.getApp().turn2LoginForLogout(); } return; }
		 */
		...
		if (!((AbsBaseActivityNetListener) mContext).hasShowTimeOutDialog) {
			...
			// WorkSignActivity.getActivity().dismissDialog();
			// Toast.makeText(mContext,mContext.getString(R.string.notifypacket_timeout),
