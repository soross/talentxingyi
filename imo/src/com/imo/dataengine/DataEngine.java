...
public class DataEngine {
	...
	public void reconnectServer() {
		if (current_logic_status == LOGICSTATUS.DISCONNECTED) {
			LogFactory.e("ERROR_NETWORK", "old connection id = " + EngineConst.IMO_CONNECTION_ID);
			...
			LogFactory.e("ERROR_NETWORK", "new connection id :" + EngineConst.IMO_CONNECTION_ID + ",old connectionID :" + oldConnectionID);
		...
		synchronized (lock_observerNotifyPacketArrived) {
			...
			while (itrObserver.hasNext()) {
				...
				if (command == IMOCommand.IMO_FORCE_EXIT || command == IMOCommand.IMO_HEART_BEAT || command == IMOCommand.IMO_SEND_MESSAGE || command == IMOCommand.IMO_MODIFY_QGROUP_NAME_NOTICE || command == IMOCommand.IMO_GET_RELOGIN || command == IMOCommand.IMO_LOGIN
					...
					if (((AbsBaseActivityNetListener) (IMOApp.getApp().mLastActivity)).CanAcceptPacket(command)) {
						((AbsBaseActivityNetListener) (IMOApp.getApp().mLastActivity)).NotifyPacketArrived(aConnectionId, command);
						break;
					}
	...
	public InPacket parseIncoming(ByteBuffer buf, int offset, int length) throws PacketParseException {
		...
		ConnectionLog.MusicLogInstance().addLog("parseIncoming packet: ," + commandRet);
		...
		switch (commandRet) {
			...
			case IMOCommand.IMO_GET_DEPT_INFO:
				GetDeptInfoInPacket deptinfoInPac = new GetDeptInfoInPacket(buf, length);
			...
			case IMOCommand.IMO_GET_ALL_EMPLOYEE_UID:
				GetAllEmployeesUIDInPacket employeesuidInPac = new GetAllEmployeesUIDInPacket(buf, length);
			...
			case IMOCommand.IMO_GET_ALL_EMPLOYEE_BASIC_INFO:
				GetAllEmployeesInfoInPacket employeesbasicinfoInPac = new GetAllEmployeesInfoInPacket(buf, length);
			...
			case IMOCommand.IMO_GET_EMPLOYEE_STATUS:
				GetEmployeesStatusInPacket employeesStatusInPac = new GetEmployeesStatusInPacket(buf, length);
			...
			case IMOCommand.IMO_GET_OFFLINE_MSG:
				GetOfflineMsgInPacket offlinemsgInPac = new GetOfflineMsgInPacket(buf, length);
			...
			case IMOCommand.IMO_GET_EMPLOYEE_PROFILE:
				GetEmployeeProfileInPacket employeeprofInPac = new GetEmployeeProfileInPacket(buf, length);
			...
			case IMOCommand.IMO_UPDATE_USER_STATUS:
				UserStatusChangeInPacket statusChangeInPac = new UserStatusChangeInPacket(buf, length);
				statusChangeInPac.setCommand(IMOCommand.IMO_UPDATE_USER_STATUS);
				LogFactory.e("user_status changed :", "cid:" + statusChangeInPac.getCid() + ",uid :" + statusChangeInPac.getUid() + ",status :" + Integer.toHexString(statusChangeInPac.getStatus()));
				removePacketFromTimeoutQueue(statusChangeInPac.get_header_seq());

				// [接到状态改变后发送ACK包]
				if (statusChangeInPac.getServer_msg_id() != 0) {
					ByteBuffer statusACKBuf = StatusAckOutPacket.GenerateStatusAckBody(statusChangeInPac.getServer_msg_id());
					StatusAckOutPacket statusAckOutPac = new StatusAckOutPacket(statusACKBuf, IMOCommand.IMO_STATUS_ACK, EngineConst.cId, EngineConst.uId);
					add(statusAckOutPac);
				}
				// [End]
			...
			case IMOCommand.IMO_EDIT_PROFILE:
				EditProfileInPacket editProfileInPac = new EditProfileInPacket(buf, length);
			...
			case IMOCommand.IMO_GET_OFFLINE_MSG_PROFILE:
				GetOfflineMsgProfileInPacket offMsgProfileInPac = new GetOfflineMsgProfileInPacket(buf, length);
			...
			case IMOCommand.IMO_GET_OFFLINE_MSG_CONTENTS:
				GetOffMsgFromContactorInPacket offMsgFromContactorInPac = new GetOffMsgFromContactorInPacket(buf, length);
				...
				for (int i = 0; i < offMsgFromContactorInPac.getOfflineMsgArray().length; i++) {
					LogFactory.e("OffContents", "" + offMsgFromContactorInPac.getOfflineMsgArray()[i].getMsg());
				}

				return offMsgFromContactorInPac;
			case IMOCommand.IMO_DELETE_OFFLINE_MSG:
				DeleteOfflineMsgInPacket deleteMsgInPac = new DeleteOfflineMsgInPacket(buf, length);
			...
			case IMOCommand.IMO_OUTER_CONTACTOR_LIST_UC:
				ContactorGroupUCInPacket groupUCInPac = new ContactorGroupUCInPacket(buf, length);
			...
			case IMOCommand.IMO_OUTER_CONTACTOR_GROUP:
				ContactorGroupInPacket groupInPac = new ContactorGroupInPacket(buf, length);
			...
			case IMOCommand.IMO_INNER_CONTACTOR_LIST:
				InnerContactorListInPacket innerlistInPac = new InnerContactorListInPacket(buf, length);
			...
			case IMOCommand.IMO_OUTER_CONTACTOR_LIST:
				OuterContactorListInPacket outerlistInPac = new OuterContactorListInPacket(buf, length);
			...
			case IMOCommand.IMO_OUTER_BASIC_INFO:
				OuterBasicInfoInPacket outerbasicinfoInPac = new OuterBasicInfoInPacket(buf, length);
			...
			case IMOCommand.IMO_UPDATE_VERSION:
				UpdateVersionInPacket updateInPac = new UpdateVersionInPacket(buf, length);
				updateInPac.setCommand(IMOCommand.IMO_UPDATE_VERSION);
				return updateInPac;
			case IMOCommand.IMO_SERVER_PROMPT:
				ServerPromptInPacket serverPromptInPac = new ServerPromptInPacket(buf, length);
				serverPromptInPac.setCommand(IMOCommand.IMO_SERVER_PROMPT);
				return serverPromptInPac;
			case IMOCommand.IMO_LOGIN_QGROUP:
				LoginQGroupInPacket loginGroupInPacket = new LoginQGroupInPacket(buf, length);
			...
			case IMOCommand.IMO_GET_USER_QGROUP_LIST:
				GetUserQGroupListInPacket getUserQGroupListInPacket = new GetUserQGroupListInPacket(buf, length);
			...
			case IMOCommand.IMO_GET_QGROUP_INFO:
				GetQGroupInfoInPacket getQGroupInfoInPacket = new GetQGroupInfoInPacket(buf, length);
			...
			case IMOCommand.IMO_GET_QGROUP_USER_LIST:
				GetQGroupUserListInPacket getQGroupUserListInPacket = new GetQGroupUserListInPacket(buf, length);
			...
			case IMOCommand.IMO_GET_USER_NGROUP_LIST_UC:
				GetUserNgGroupListUcInPacket getUserNgGroupListUcInPacket = new GetUserNgGroupListUcInPacket(buf, length);
			...
			case IMOCommand.IMO_GET_USER_NGROUP_LIST_UC_NEW:
				GetUserNgGroupListUcNewInPacket getUserNgGroupListUcNewInPacket = new GetUserNgGroupListUcNewInPacket(buf, length);
			...
			case IMOCommand.IMO_CLIENT_LOGIN_NGROUPD:
				GetClientLoginNgroupdInPacket getClientLoginNgroupdInPacket = new GetClientLoginNgroupdInPacket(buf, length);
			...
			case IMOCommand.IMO_AGREE_JOIN_NGROUP:
				GetAgreeJoinNgroupInPacket getAgreeJoinNgroupInPacket = new GetAgreeJoinNgroupInPacket(buf, length);
			...
			case IMOCommand.IMO_NOTICE_USER_JOIN_NGROUP:
				GetNoticeUserJoinNgroupInPacket getNoticeUserJoinNgroupInPacket = new GetNoticeUserJoinNgroupInPacket(buf, length);
			...
			case IMOCommand.IMO_NGROUP_MODIFY_ANNOUNCEMENT_NOTICE:
				GetNgroupModifyAnnouncementNoticeInPacket getNgroupModifyAnnouncementNoticeInPacket = new GetNgroupModifyAnnouncementNoticeInPacket(buf, length);
			...
			case IMOCommand.IMO_GET_NGROUP_INFO:
				GetNgroupInfoInPacket getNgroupInfoInPacket = new GetNgroupInfoInPacket(buf, length);
			...
			case IMOCommand.IMO_GET_USER_NGROUP_LIST:
				GetUserNgroupListInPacket getUserNgroupListInPacket = new GetUserNgroupListInPacket(buf, length);
			...
			case IMOCommand.IMO_GET_NGROUP_USER_LIST:
				GetNgroupUserListInPacket getNgroupUserListInPacket = new GetNgroupUserListInPacket(buf, length);
			...
			case IMOCommand.IMO_GET_NGROUP_USERS_STATUS:
				GetNgroupUsersStatusInPacket getNgroupUsersStatusInPacket = new GetNgroupUsersStatusInPacket(buf, length);
			...
			case IMOCommand.IMO_NGOURP_CHAT:
				GetNgroupChatInPacket getNgroupChatInPacket = new GetNgroupChatInPacket(buf, length);
			...
			case IMOCommand.IMO_NEW_MSG_NOTICE_NGROUP:
				GetNewMsgNoticeNgroupInPacket getNewMsgNoticeNgroupInPacket = new GetNewMsgNoticeNgroupInPacket(buf, length);
			...
			case IMOCommand.IMO_NGROUP_USER_CHANGE_STATUS_NOTICE:
				GetNgroupUserChangeStatusNoticeInPacket getNgroupUserChangeStatusNoticeInPacket = new GetNgroupUserChangeStatusNoticeInPacket(buf, length);
			...
			case IMOCommand.IMO_EXIT_NGROUP:
				GetExitNgroupInPacket getExitNgroupInPacket = new GetExitNgroupInPacket(buf, length);
			...
			case IMOCommand.IMO_EXIT_NGROUP_NOTICE_USERS:
				GetExitNgroupNoticeUsersInPacket getExitNgroupNoticeUsersInPacket = new GetExitNgroupNoticeUsersInPacket(buf, length);
			...
			case IMOCommand.IMO_MODIFY_NGROUP_NAME_NOTICE:
				GetModifyNgroupNameNoticeInPacket getModifyNgroupNameNoticeInPacket = new GetModifyNgroupNameNoticeInPacket(buf, length);
			...
			case IMOCommand.IMO_NGROUP_KICK_USER_NOTICE:
				GetNgroupKickUserNoticeInPacket getNgroupKickUserNoticeInPacket = new GetNgroupKickUserNoticeInPacket(buf, length);
			...
			case IMOCommand.IMO_GET_OFFLINE_NGROUP_CHAT_MSG:
				GetOffLineNgroupChatMsgInPacket getOffLineNgroupChatMsgInPacket = new GetOffLineNgroupChatMsgInPacket(buf, length);
			...
			case IMOCommand.IMO_NGROUP_NOTICE:
				GetNgroupNoticeInPacket getNgroupNoticeInPacket = new GetNgroupNoticeInPacket(buf, length);
			...
			case IMOCommand.IMO_QGOURP_CHAT:
				QGroupChatInPacket qGroupChatInPacket = new QGroupChatInPacket(buf, length);
			...
			case IMOCommand.IMO_NEW_MSG_NOTICE_QGROUP:
				NewMsgNoticeQGroupInPacket newMsgNoticeQGroupInPacket = new NewMsgNoticeQGroupInPacket(buf, length);
			...
			case IMOCommand.IMO_QGROUP_USER_CHANGE_STATUS_NOTICE:
				QGroupUserChangeStatusNoticeInPacket qGroupUserChangeStatusNoticeInPacket = new QGroupUserChangeStatusNoticeInPacket(buf, length);
			...
			case IMOCommand.IMO_EXIT_QGROUP:
				ExitQgroupInPacket exitQgroupInPacket = new ExitQgroupInPacket(buf, length);
			...
			case IMOCommand.IMO_EXIT_QGROUP_NOTICE_USERS:
				ExitQgroupNoticeUsersInPacket exitQgroupNoticeUsersInPacket = new ExitQgroupNoticeUsersInPacket(buf, length);
			...
			case IMOCommand.IMO_MODIFY_QGROUP_NAME_NOTICE:
				ModifyQgroupNameNoticeInPacket modifyQgroupNameNoticeInPacket = new ModifyQgroupNameNoticeInPacket(buf, length);
			...
			case IMOCommand.IMO_DESTORY_THE_QGROUP:
				DestoryTheQgroupInPacket destoryTheQgroupInPacket = new DestoryTheQgroupInPacket(buf, length);
			...
			case IMOCommand.IMO_QGROUP_KICK_USER_NOTICE:
				QGroupKickUserNoticeInPacket qcGroupKickUserNoticeInPacket = new QGroupKickUserNoticeInPacket(buf, length);
			...
			case IMOCommand.IMO_GET_QGROUP_OFFLINE:
				GetQGroupOfflineInPacket getQGroupOfflineInPacket = new GetQGroupOfflineInPacket(buf, length);
			...
			case IMOCommand.IMO_GET_OFFLINE_QGROUP_CHAT_MSG:
				GetOfflineQGroupChatMsgInPacket getOfflineQGroupChatMsgInPacket = new GetOfflineQGroupChatMsgInPacket(buf, length);
			...
			case IMOCommand.IMO_DELETE_QGROUPD_OFFLINE:
				DeleteQGroupOfflineInPacket deleteQGroupOfflineInPacket = new DeleteQGroupOfflineInPacket(buf, length);
			...
			case IMOCommand.IMO_QGROUP_DESTROY_NOTICE_USER:
				QgroupDestroyNoticeUserInPacket qgroupDestroyNoticeUserInPacket = new QgroupDestroyNoticeUserInPacket(buf, length);
