...
public class ConnectionChangeReceiver extends BroadcastReceiver {
	...
	public static boolean pingAddress() {
		...
		try {
			// ping功能
			// status
			// =InetAddress.getByName(EngineConst.hostName).isReachable(timeOut);
		...
		/*
		 * boolean mPingIpAddrResult = false; try { // This is hardcoded IP
		 * addr. This is for testing purposes. // We would need to get rid of
		 * this before release. Log.e("ping", "pingAddress 1"); Process p =
		 * Runtime.getRuntime().exec( "ping -c 1 -w 3 " + EngineConst.hostIP);
		 * Log.e("ping", "pingAddress 2"); int status = p.waitFor();
		 * Log.e("ping", "pingAddress 3"); if (p != null) p.destroy();
		 * Log.e("ping", "pingAddress 4"); if (status == 0) { mPingIpAddrResult
		 * = true; Log.e("pingAddress", "Status = " + mPingIpAddrResult); return
		 * mPingIpAddrResult; } else { mPingIpAddrResult = false;
		 * Log.e("pingAddress", "Status = " + mPingIpAddrResult); return
		 * mPingIpAddrResult; } } catch (IOException e) { mPingIpAddrResult =
		 * false; Log.e("pingAddress", "Status = " + mPingIpAddrResult); return
		 * mPingIpAddrResult; } catch (InterruptedException e) {
		 * mPingIpAddrResult = false; Log.e("pingAddress", "Status = " +
		 * mPingIpAddrResult); return mPingIpAddrResult; }
		 */
	}
