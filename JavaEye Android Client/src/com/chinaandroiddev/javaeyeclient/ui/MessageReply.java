...
public class MessageReply extends Activity {
		...
		msgButtonSend.setOnClickListener(new Button.OnClickListener() {
			...
			public void onClick(View v) {
				if (msgReply.getText().toString().length() <= 0) {
					FlurryAgent.onError("Message Reply Error",
							"message body cannot be empty", LOG_TAG);
					new AlertDialog.Builder(MessageReply.this).setTitle("警告")
							.setMessage("短信回复不能为空!")
	...
	private void sendMsg() {
		progressDialog = ProgressDialog.show(MessageReply.this, "请稍等...",
				"短信发送中...", true);
		new Thread() {
			public void run() {
				...
				try {
					...
					FlurryAgent.onError("Message Reply Error", e.getMessage(),
							LOG_TAG);
					m = null;
				}
				if (m == null) {
