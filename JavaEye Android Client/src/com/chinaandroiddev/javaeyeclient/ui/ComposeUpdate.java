...
public class ComposeUpdate extends Activity {
		...
		sendButton.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				if (msg.getText().toString().length() > 0
						&& msg.getText().toString().length() <= 140) {
					createUpdate();
				} else {
					Map<String, String> map = new HashMap<String, String>();
					map.put("failure_desc",
							"message length is 0 or more than 140");
					FlurryAgent.onEvent("Compose Update Failure", map);
					new AlertDialog.Builder(ComposeUpdate.this).setTitle("警告")
	...
	private void createUpdate() {
		progressDialog = ProgressDialog.show(ComposeUpdate.this, "请稍等...",
				"发布新闲聊中...", true);
		new Thread() {
			public void run() {
				Update u = new Update();
				...
				try {
					...
					FlurryAgent.onError("Create Update Error", e.getMessage(),
							LOG_TAG);
					success = false;
				}
